...
import fitz  # PyMuPDF
import pandas as pd
import xml_facturas as xf   # motor XML → PDF (reglas, sin API)
# ============================================================
...
# ============================================================
# Viven en UN solo lugar: xml_facturas.py (sección "LISTAS DE RUCs CONOCIDOS").
# Ahí se agregan los RUCs nuevos y aplican tanto a imágenes/PDF como a XML.
from xml_facturas import (RUCS_BANCOS, RUCS_COMBUSTIBLE, RUCS_RESTAURANTES, RUCS_SEGUROS,
                          RUCS_SERVICIOS_PUBLICOS, RUCS_BIENES, RUCS_SERVICIOS, RUCS_MIXTO_GRIFO,
                          categoria_por_nombre_emisor, categoria_por_descripcion)
...
PDF_EXT   = {'.pdf'}
XML_EXT   = {'.xml'}
ALL_EXT   = IMAGE_EXT | PDF_EXT | XML_EXT
...
- restaurante_consumo: Comida o bebida preparada para consumir en el momento — restaurantes, fast food, cafeterías, bares, heladerías, pizzerías, chifas, pollerías, food courts y tiendas de conveniencia dentro de grifos (ej. "Listo" de Primax). Señales: platos preparados (lomo saltado, ceviche, chaufa meloso, pollo a la brasa), fast food (combos, hamburguesas, Whopper, nuggets, alitas, tacos), pizzas ("13 Familiar Borde Queso Honolulu", "Deluxe"), postres y helados (frozen, yogurt, waffle), cafés (cappuccino, latte) y gaseosas de consumo inmediato (Coca Cola 300ml, Inka Cola).
  MUY IMPORTANTE: esto aplica AUNQUE los ítems estén listados como productos con cantidad y precio unitario, y AUNQUE la unidad de medida diga SERVICIO. Un "Combo HD Grill + Coca Cola 300ml" o un "Tennesse Burger (Lunch)" es restaurante_consumo, NO es "bien" ni "servicio".
  Fíjate también en el emisor: muchas franquicias facturan con una razón social que no dice nada pero el nombre comercial sí (DELOSI = KFC, SAIDEL = Burger King, PINKDEL = Pinkberry, ALERT DEL PERU = China Wok, OPERACIONES ARCOS DORADOS = McDonald's, FRANQUICIAS ALIMENTARIAS = Fridays, DPP CORP = pizzas). Todas son restaurante_consumo.
...
- servicio: Servicio SIN detracción: internet, alquiler, mantenimiento, consultoría, limpieza, seguridad, transporte, agencias de viaje, tasas turísticas, lavandería y tintorería. OJO con las lavanderías: facturan la prenda del cliente ("BLUSA DE SEDA", "TERNO"), pero lo que cobran es el lavado → es servicio, NO bien. Si la razón social dice LAVANDERIA, TINTORERIA, PELUQUERIA o similar, es servicio.
- bien: Productos físicos para llevar/revender/abastecer, comprados en supermercados (Wong, Plaza Vea, Metro), minimarkets, tiendas por departamento (Saga Falabella, Ripley) o tiendas Tambo/Mass — insumos dentales, farmacéuticos, ropa, calzado, electrodomésticos, materiales, abarrotes para preparar en casa. Tienen lista de productos con cantidad y precio unitario Y NO hay sección de detracción.
...
2. Si no hay detracción, revisa si el emisor es un negocio de comida o si los ítems son comida/bebida preparada → restaurante_consumo (esto vence a "bien" y a "servicio")
3. Luego revisa si el emisor presta un servicio (lavandería, alquiler, consultoría...) → servicio
4. Solo si no es ninguna de las anteriores y hay tabla de productos con cantidad/precio → bien
...
def _tiene_detraccion_en_texto(pdf_path: Path) -> bool:
    ...
    return hits >= 2
def _texto_documento(path: Path) -> str:
    """Texto del PDF (vacío para imágenes escaneadas). Se usa para aplicar las mismas
    reglas de descripción que se usan con los XML, sin gastar API."""
...
def a_base64(path):
    ...
    if ext in IMAGE_EXT:
        ...
        # FIX DEPLOY: se usa fitz (PyMuPDF) en vez de pdf2image/poppler.
        # fitz es 100% Python (no requiere apt-get / poppler-utils), asi
        # se evita depender de packages.txt y del bug de apt en Streamlit Cloud.
        try:
            doc = fitz.open(str(path))
    ...
    try:
        ...
        if ruc in RUCS_BANCOS:
            ...
            if result.get('tipo_documento') not in ('guia_remision', 'nota_pedido', 'otro', 'recibo_honorarios'):
                ...
                result['tipo_documento'] = 'factura'
        elif ruc in RUCS_MIXTO_GRIFO:
            # Grifo con tienda de conveniencia: decide la descripción, no el RUC
            nombre_mix, por_defecto = RUCS_MIXTO_GRIFO[ruc]
            result['nombre_emisor'] = result.get('nombre_emisor') or nombre_mix
            por_desc = categoria_por_descripcion(_texto_documento(path))
            result['categoria'] = por_desc[0] if por_desc else por_defecto
            if result.get('tipo_documento') not in ('nota_credito', 'nota_debito'):
                result['tipo_documento'] = 'factura'
        else:
            # RUC desconocido: mismas reglas generales que se aplican a los XML.
            # 1) La razón social / nombre comercial del emisor (DELOSI = KFC, LAVANDERIA = servicio)
            # 2) La descripción de los ítems (combos, hamburguesas, pizzas, gaseosas = consumo)
            # Solo corrigen cuando el modelo dijo 'bien' o 'servicio', que son los dos
            # cajones donde caía por error la comida preparada.
            if result.get('categoria') in ('bien', 'servicio') and not result.get('tiene_detraccion'):
                texto_doc = _texto_documento(path)
                por_desc = categoria_por_descripcion(texto_doc)
                por_nombre = categoria_por_nombre_emisor(result.get('nombre_emisor') or '', texto_doc[:400])
                if por_desc and por_desc[0] == 'restaurante_consumo':
                    result['categoria'] = 'restaurante_consumo'
                    result['razon'] = por_desc[1]
                elif por_nombre:
                    result['categoria'] = por_nombre[0]
                    result['razon'] = por_nombre[1]
...
def carpeta_destino(result, base_path):
    ...
    return cur / 'Bienes'
def _norm_serie(serie):
    """F672-00603536 y F672-603536 son el mismo comprobante (XML vs impresión)."""
    s = (serie or '').strip().upper().replace(' ', '')
    m = re.fullmatch(r'([A-Z0-9]+)-0*(\d+)', s)
...
st.caption("Soporta JPG, JFIF, PNG, PDF (incluso escaneados y multi-página) y XML de SUNAT. "
           "Los XML se convierten a un PDF legible tipo factura y se clasifican por reglas, sin gastar API.")
st.divider()
...
with st.expander("⚙️ Opciones para XML"):
    incluir_xml   = st.checkbox("Guardar el XML original junto al PDF generado", value=True)
    ia_xml        = st.checkbox("Usar IA también para afinar la categoría de los XML de emisores desconocidos", value=False,
                                help="Solo texto de los ítems (muy barato). Sin esto los XML se clasifican 100% por reglas.")
    validar_cli   = st.checkbox("Alertar si el RUC cliente del XML no es el de mi empresa", value=True)
st.divider()
st.subheader("📤 Sube tus documentos")
archivos_subidos = st.file_uploader(
    "Arrastra o selecciona archivos (JPG, JFIF, PNG, PDF, XML) o un ZIP con todos.\n"
    "Los PDFs con varios comprobantes se separan por página. Los XML generan su propio PDF.",
    type=["jpg", "jpeg", "jfif", "png", "webp", "bmp", "pdf", "xml", "zip"],
...
if st.button("🚀 Clasificar documentos", type="primary", use_container_width=True):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        archivos_raw = sorted([f for f in entrada.rglob('*') if f.suffix.lower() in ALL_EXT and not f.name.startswith('._')])
        if not archivos_raw:
            st.error("No se encontraron archivos de imagen, PDF o XML.")
            st.stop()
        hay_visuales = any(f.suffix.lower() != '.xml' for f in archivos_raw)
        if hay_visuales and client is None:
            st.error("Hay imágenes/PDF: ingresa tu API Key de Anthropic (los XML solos no la necesitan).")
            st.stop()
        ...
        for i, f in enumerate(archivos):
            ...
            # ---------- XML: parseo por reglas + PDF generado ----------
            if f.suffix.lower() == '.xml':
                c = xf.parse_xml(f)
                if c.error:
                    copiar_seguro(f, salida / 'No Procesados')
                    resultados.append({'Archivo': f.name, 'Carpeta': 'No Procesados', 'Estado': c.error})
                    continue
                xf.clasificar(c)
                if ia_xml and client is not None:
                    xf.afinar_con_claude(c, client, mi_ruc.strip())
                alerta_fecha = False
                try:
                    alerta_fecha = datetime.strptime(c.fecha_emision, '%Y-%m-%d').strftime('%m') != mes_trabajo.strip()
                except Exception:
                    pass
                clave = c.ruc_emisor + '|' + _norm_serie(c.serie_numero)
                es_dup = clave in vistos
                if es_dup:
                    dest = salida / 'Duplicados'
                else:
                    vistos[clave] = f.name
                    dest = xf.carpeta_destino(c, salida)
                rel = str(dest.relative_to(salida))
                pdf_path = dest / xf.nombre_pdf(c)
                n = 1
                while pdf_path.exists():
                    pdf_path = dest / f"{xf.nombre_pdf(c)[:-4]}_{n}.pdf"; n += 1
                try:
                    xf.generar_pdf(c, pdf_path, rel, mi_ruc.strip())
                except Exception as ex:
                    copiar_seguro(f, salida / 'No Procesados')
                    resultados.append({'Archivo': f.name, 'Carpeta': 'No Procesados', 'Estado': f'Error PDF: {ex}'})
                    continue
                if incluir_xml:
                    pdf_path.with_suffix('.xml').write_bytes(f.read_bytes())
                resultados.append({
                    'Archivo'         : f.name,
                    ...
                    'Moneda'          : c.moneda,
                    'Serie / Numero'  : c.serie_numero,
                    'Fecha Emision'   : c.fecha_emision,
                    'Monto Total'     : float(c.importe_total),
                    'IGV'             : float(c.igv),
                    'Tiene IGV'       : c.tiene_igv,
                    'Tiene Detraccion': c.tiene_detraccion,
                    'Monto Detraccion': float(c.detraccion_monto),
                    'Tiene Percepcion': c.tiene_percepcion,
                    'Monto Percepcion': float(c.percepcion_monto),
                    'Cliente RUC'     : c.doc_cliente,
                    'Alerta Cliente'  : bool(validar_cli and c.doc_cliente and c.doc_cliente != mi_ruc.strip()),
                    'Carpeta'         : rel,
                ...
                continue
            # ---------- Imagen / PDF: Claude visión (flujo original) ----------
        ...
        with pd.ExcelWriter(excel_buf, engine='openpyxl') as writer:
            ...
            for hoja, col in [('Alerta Cliente', 'Alerta Cliente'), ('Con Detraccion', 'Tiene Detraccion'), ('Con Percepcion', 'Tiene Percepcion')]:
                if col in df.columns:
                    sub = df[df[col] == True]
                    if not sub.empty:
                        sub.to_excel(writer, sheet_name=hoja, index=False)
        excel_buf.seek(0)
...
if 'resultados' in st.session_state:
    ...
    cli_count = int(df['Alerta Cliente'].fillna(False).astype(bool).sum()) if 'Alerta Cliente' in df.columns else 0
    if cli_count:
        st.warning(f"🚩 {cli_count} XML cuyo RUC cliente no es {st.session_state['mi_ruc']}.")
    if 'Origen' in df.columns:
        n_xml = int((df['Origen'] == 'XML').sum()); n_img = int((df['Origen'] == 'Imagen/PDF').sum())
        st.caption(f"Origen: {n_xml} desde XML (PDF generado) · {n_img} desde imagen/PDF (IA visión)")
    st.subheader("Resumen")
    cols_show = [c for c in ['Archivo', 'Origen', 'Emisor', 'Moneda', 'Tipo', 'Categoria', 'Carpeta', 'Monto Total'] if c in df.columns]
